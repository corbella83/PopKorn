...
C) To be used anywhere like this:
```swift
...
Using JS
--------
PopKorn provides basic support to JS platform. In your multiplatform project, write a File (Bridge.kt) on your JS module and add this 2 functions: 
```kotlin
fun init(){
...
fun getInjector() = InjectorJS(popKorn())
```

You will have to initialize PopKorn somewhere (BridgeKt.init()) and use it with InjectorJS

We are still working in providing full support to JS
